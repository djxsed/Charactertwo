...
postgresql://username:password@host:port/dbname # 형식 가정
...
숫자 속성 체크용 정규 표현식
NUMBER_PATTERN = r"\b(체력|지능|이동속도|힘)\s*:\s*([1-6])\b|\b냉철\s*:\s*([1-4])\b|<math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mstyle mathcolor="#cc0000"><mtext>\w</mtext></mstyle><mo>+</mo></mrow><annotation encoding="application/x-tex">\w+</annotation></semantics></math>\s*<math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mo stretchy="false">(</mo><mstyle mathcolor="#cc0000"><mtext>\d</mtext></mstyle><mo stretchy="false">)</mo></mrow><annotation encoding="application/x-tex">(\d)</annotation></semantics></math>"
...
{"field": "사용 기술/마법/요력 위력", "prompt": "사용 기술/마법/요력의 위력을 선택해주세요.", "options": ["1", "2", "3", "4", "5", "6"], "error_message": "위력은 1에서 6 사이의 숫자여야 합니다.", "is_tech": True},
{"field": "사용 기술/마법/요력 쿨타임", "prompt": "사용 기술/마법/요력의 쿨타임을 입력해주세요. (예: 30초, 최소 위력 4는 15초, 위력 5는 20초, 위력 6은 40초로 해주세요.)", "validator": lambda x: len(x) > 0, "error_message": "쿨타임을 입력해주세요.", "is_tech": True},
{"field": "사용 기술/마법/요력 지속시간", "prompt": "사용 기술/마법/요력의 지속시간을 입력해주세요. (예: 10초, 할퀴기나 주먹같은 단발 공격이스 1초로 해주세요)", "validator": lambda x: len(x) > 0, "error_message": "지속시간을 입력해주세요.", "is_tech": True},
{"field": "사용 기술/마법/요력 설명", "prompt": "사용 기술/마법/요력을 설명해주세요. (최소 20자)", "validator": lambda x: len(x) >= 20, "error_message": "설명이 너무 짧습니다. 최소 20자 이상 입력해주세요.", "is_tech": True},
{"field": "사용 기술/마법/요력 추가 여부", "prompt": "기술/마법/요력을 추가하시겠습니까?", "options": ["예", "아니요"], "error_message": "예 또는 아니요로 선택해주세요."},
{"field": "과거사", "prompt": "과거사를 설명해주세요. (최소 20자)", "validator": lambda x: len(x) >= 20, "error_message": "과거사 설명이 너무 짧습니다. 최소 20자 이상 입력해주세요."},
{"field": "특징", "prompt": "특징을 설명해주세요. (최소 10자)", "validator": lambda x: len(x) >= 10, "error_message": "특징 설명이 너무 짧습니다. 최소 10자 이상 입력해주세요."},
...
print(f"Found 캐릭터-목록 channel: {char_channel.name} (ID: {char_channel.id}, Type: {type(char_channel).name})")
try:
...
if question.get("condition") and not question"condition":
continue
while True:
...
field = f"{tech_question['field']}{tech_counter}"
while True:
...
message = await send_message_with_retry(channel, f"{tech_question['prompt']}", view=view)
view.message = message
await view.wait()
...
try:
response = await bot.wait_for("message", check=check, timeout=600.0)
tech_answer = response.content.strip() if response.content.strip() else f"이미지{response.attachments[0].url}" if response.attachments else ""
if tech_question"validator":
...
message = await send_message_with_retry(channel, f"{user.mention} 기술/마법/요력을 추가하시겠습니까?", view=add_more_view)
...
banned_words=', '.join(BANNTABLE_WORDS),
required_fields=', '.join(REQUIRED_FIELDS),
allowed_races=', '.join(DEFAULT_ALLOWED_RACES),
allowed_roles=', '.join(allowed_roles),
description=description
...
techs = [(k, answers[k], answers.get(f"사용 기술/마법/요력 위력_{k.split('')[1]}"), answers.get(f"사용 기술/마법/요력 쿨타임{k.split('')[1]}"), answers.get(f"사용 기술/마법/요력 지속시간{k.split('')[1]}"), answers.get(f"사용 기술/마법/요력 설명{k.split('')[1]}"))
for k in sorted([k for k in answers if k.startswith("사용 기술/마법/요력")], key=lambda x: int(x.split('_')[1]))]
...
if tech_question.get("is_tech"):
while True:
field = f"{tech_question['field']}{techs[idx][0].split('')[1]}"
if tech_question.get("options"):
view = SelectionView(tech_question["options"], field, user, lambda option: handle_selection(field, option))
...
key = techs[idx][0]
del answers[key]
del answers[f"사용 기술/마법/요력 위력_{key.split('')[1]}"]
del answers[f"사용 기술/마법/요력 쿨타임{key.split('')[1]}"]
del answers[f"사용 기술/마법/요력 지속시간{key.split('')[1]}"]
del answers[f"사용 기술/마법/요력 설명{key.split('_')[1]}"]
else:
await send_message_with_retry(channel, f"{user.mention} ❌ 유효한 번호를 입력해줘! 다시 시도해~ 🥹")
except (ValueError, asyncio.TimeoutError):
await send_message_with_retry(channel, f"{user.mention} ❌ 잘못된 입력이거나 시간이 초과됐어! 다시 시도해~ 🥹")
